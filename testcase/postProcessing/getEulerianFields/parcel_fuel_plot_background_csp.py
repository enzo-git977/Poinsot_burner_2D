...
# choice of parcel (66,100)
target_parcel_id = 100
print(f"Tracking fuel parcel number : {target_parcel_id} ")
...
for time_str in time_folders:
    file_path = os.path.join(base_dir, time_str, 'fuelCloud_withCSP.dat')
...
for i in range(len(x_positions)):
    ...
    # Save the image
    #plt.tight_layout()
    fig.savefig(f"pictures_fuel_csp/fuel_parcel_{i:04d}.png", dpi=150)
    plt.close(fig)
    print(f"Plotting successful for t={time_stamps[i]:.4f}")
print("Pictures.png saves in GetEulerianFields/pictures_fuel_csp")
